...
![Сжатие раздела Windows перед установкой Manjaro](windows-shrink-volume.jpg)
...
![Нераспределённое место рядом с Windows](windows-disk-unallocated.jpg)
...
Отдельный swap-раздел в этом маршруте **не создаём**.
...
Итоговая разметка должна выглядеть примерно так:

![Итоговая ручная разметка dual boot: отдельный EFI Manjaro и root рядом с Windows](dualboot-manual-partition.jpg)
...
После перезагрузки должно появиться меню, в котором можно запустить Manjaro или Windows.

Сначала загрузите Manjaro. Затем перезагрузитесь и проверьте запуск Windows.

### Если Windows не появилась в меню Manjaro

Загрузите Manjaro и выполните:
...
После перезагрузки Windows должна появиться в списке. Если нет, Windows Boot Manager всё равно можно выбрать через Boot Menu материнской платы.
